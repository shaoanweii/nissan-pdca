...
const handleChange = (val: any) => {
  ...
  if (val.length >= cacheValue.value?.length) {
    if (
      !(val.includes('all') || val.includes('全部意图')) &&
    ...
    if (
      !(val.includes('all') || val.includes('全部意图')) &&
      allList.includes(cacheValue.value?.[0])
    ) {
      setValue([])
      return
    }
    if (val.includes('all') || val.includes('全部意图')) {
...
const handleOptions = () => {
  ...
  if (fieldNames.value) {
    allObj = { [fieldNames.value.value!]: 'all', [fieldNames.value.label!]: '全部意图' }
  } else {
    allObj = { value: 'all', label: '全部意图' }
  }

  if (options.value?.length > 0) {
    const firstLabel = options.value[0][fieldNames.value.label!]
    if (firstLabel === '全部意图') return
